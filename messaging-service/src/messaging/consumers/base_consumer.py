...
class NetworkConsumer(BaseConsumer):
    ...
    def _handle_donation_offer(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            # Process the external offer
            success = offer_service.process_external_offer(message_data)
            
            if success:
                logger.info("Donation offer processed successfully", offer_id=message_data.get('offer_id'))
            else:
                logger.error("Failed to process donation offer", offer_id=message_data.get('offer_id'))
                
        except Exception as e:
            logger.error("Error handling donation offer", error=str(e), offer_id=message_data.get('offer_id'))
    ...
    def _handle_solidarity_event(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            success = event_service.process_external_event(message_data)
            
            if success:
                logger.info("Solidarity event processed successfully", event_id=message_data.get('event_id'))
            else:
                logger.error("Failed to process solidarity event", event_id=message_data.get('event_id'))
                
        except Exception as e:
            logger.error("Error handling solidarity event", error=str(e), event_id=message_data.get('event_id'))
    ...
    def _handle_event_cancellation(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            # Process the event cancellation
            success = event_service.process_event_cancellation(message_data)
            
            if success:
                logger.info("Event cancellation processed successfully", event_id=message_data.get('event_id'))
            else:
                logger.error("Failed to process event cancellation", event_id=message_data.get('event_id'))
                
        except Exception as e:
            logger.error("Error handling event cancellation", error=str(e), event_id=message_data.get('event_id'))
...
class OrganizationConsumer(BaseConsumer):
    ...
    def _handle_event_adhesion(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            if success:
                logger.info("Event adhesion processed successfully", event_id=message_data.get('event_id'))
            else:
                logger.error("Failed to process event adhesion", event_id=message_data.get('event_id'))
                
        except Exception as e:
            logger.error("Error handling event adhesion", error=str(e), event_id=message_data.get('event_id'))