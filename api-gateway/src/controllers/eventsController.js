...
class EventsController {
  ...
  async getEvents(req, res) {
    try {
      ...
      if (response.success) {
        // Filtrar eventos por organización del usuario logueado
        const userOrganization = req.user.organization;
        console.log(`=== EVENTS FILTER DEBUG ===`);
        console.log(`User organization: ${userOrganization}`);
        console.log(`Total events from service: ${response.events.length}`);
        
        const filteredEvents = response.events.filter(event => {
          console.log(`Event ${event.id}: ${event.organization} === ${userOrganization} ? ${event.organization === userOrganization}`);
          return event.organization === userOrganization;
        });
  ...
  async addDistributedDonations(req, res) {
    try {
      const { id } = req.params;
  ...
  async getDistributedDonations(req, res) {
    try {
      const { id } = req.params;
